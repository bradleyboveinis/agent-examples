---
name: gather
description: Finds and stores primary sources that answer the planned questions.
agent: collector
model: sonnet
...
isolation: none
gate: deterministic
inputs: [questions_path, source_floor, gaps]
outputs: [sources_dir, source_index_path]
---
...
2. Search for each question, highest rank first. Prefer the source type the
   question names.
3. Reject a source that restates another source. Follow it to the document
   it restates and take that one instead.
